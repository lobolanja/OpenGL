...
void material (void) {
  ...
  GLfloat color_difuso_y_ambiente[] = {0.8, 0.8, 0.8, 1.0};
  GLfloat color_especular[]         = {0.1, 0.5, 0.8, 1.0};
...
void iluminacion() {
  ...
  GLfloat posicion[]=  {0.0f, 5.0f, 5.0f, 1.0f};
  GLfloat difusa[]=    {0.7f, 0.7f, 0.7f, 1.0f};
  GLfloat especular[]= {0.2f, 0.2f, 0.2f, 1.0f};
  GLfloat ambiente[]=  {0.2f, 0.2f, 0.2f, 1.0f};

  GLfloat luz_ambiente_ninguna[] = {0.0, 0.0, 0.0, 1.0};
  GLfloat luz_ambiente_suave[]   = {0.2, 0.2, 0.2, 1.0};
  GLfloat luz_ambiente_fuerte[]  = {0.6, 0.2, 0.2, 1.0};

  switch (imagenActual) {
  case 0 : glLightModelfv(GL_LIGHT_MODEL_AMBIENT, luz_ambiente_ninguna);
  	   break;
  case 1 : glLightModelfv(GL_LIGHT_MODEL_AMBIENT, luz_ambiente_suave);
     	   break;
  case 2 : glLightModelfv(GL_LIGHT_MODEL_AMBIENT, luz_ambiente_fuerte);
     	   break;
  }
